...
import { X, ArrowRight, User, Heart, ShoppingCart } from "lucide-react"
...
export function MobileMenu({ isOpen, wishlist, cart, onClose }: MobileMenuProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          <nav className="space-y-6">
            <div className="space-y-2">
              ...
              <ul className="space-y-4">
                {["Home", "Shop", "Categories", "New Arrivals", "Sale"].map((item, index) => (
                  <motion.li
                    key={item}
                  ...
                  >
                    <Link
                      href="#"
                      className="text-lg font-medium hover:text-pink-400 transition-colors flex items-center justify-between"
                    >
                      {item}
            ...
            <div className="space-y-2">
              ...
              <ul className="space-y-4">
                {[
                  { name: "My Account", icon: <User className="h-4 w-4" /> },
                  { name: `Wishlist (${wishlist.length})`, icon: <Heart className="h-4 w-4" /> },
                  { name: `Cart (${cart.length})`, icon: <ShoppingCart className="h-4 w-4" /> },
                ].map((item, index) => (
                  <motion.li
                    key={item.name}
                    initial={{ opacity: 0, x: -20 }}
