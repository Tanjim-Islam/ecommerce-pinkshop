...
export function QuickView({ product, wishlist, cart, toggleWishlist, toggleCart, closeQuickView }: QuickViewProps) {
              ...
              <div className="space-y-4">
                    ...
                    {productColors.map((color, index) => (
                      <button
                        ...
                        className={`w-8 h-8 rounded-full ${color.value} flex items-center justify-center ${
                          selectedColor === index ? "ring-2 ring-pink-400 ring-offset-2" : ""
                        }`}
                        onClick={() => setSelectedColor(index)}
                        aria-label={`Select ${color.name} color`}
                      >
                        {selectedColor === index && <Sparkles className="h-4 w-4 text-white" />}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center border border-gray-300 rounded-full w-fit">
                    ...
                    <span className="px-3 py-2 text-gray-700 min-w-[40px] text-center">{quantity}</span>
                    <button
              ...
              <div className="border-t pt-4">
                <Tabs defaultValue="description">
                  ...
                  <TabsContent value="shipping" className="pt-4 text-gray-600">
                    <p>
                      Free standard shipping on all orders over $50. Expedited and international shipping options
                      available at checkout. Returns accepted within 30 days of delivery.
                    </p>
                  </TabsContent>
                </Tabs>
              </div>
